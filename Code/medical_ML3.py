...
#        lb = LabelBinarizer()
    ...
    def predict(self, est, cv, model_name, scoring=None, model='regression', verbose=False, imputer = 'iterative', add_missing_flags = True):
        """
...
#                                return_estimator = True)
        ...
        if hasattr(est_pipe.named_steps['predictor'], 'best_params_'):
            ...
            self.save_scores(est_pipe.named_steps['predictor'].best_params_, filepath = self.result_dir + "/" + model_name + "_best_params.json")
...
#                coef_dataframe.to_csv('{}/{}_coefficients.csv'.format(self.result_dir, model_type), index = False)
            ...
            if model_type == 'PCE':
                ...
                roc_auc = auc(fpr, tpr)
                sdc, lc, _, hc = calc_auc_conf_interval(roc_auc, (pred_probs.ascvdany5y == 0).sum(), (pred_probs.ascvdany5y ==1).sum())
                plt.plot(fpr, tpr,
                         label=r'PCE: %0.3f (%0.3f-%0.3f)' % (roc_auc, lc, hc),
                         lw=2, alpha=.5)
                mean_auc = roc_auc
            else:    
                for i in range(cv):
...
#        self.leg_colors = {handle.get_label(): handle for handle in leg.legendHandles}
        ...
        if train:
            self.plot_train(model_types, cv, pce_file, title = tr_title)
        if test:
            self.save_and_plot_test_results(model_types, cv, pce_file, test_pce_file, self.result_dir, title = test_title)
