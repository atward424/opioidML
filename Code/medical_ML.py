...
#                                return_estimator = True)
        ...
        est_pipe.fit(X.as_matrix(), y)
        self.save_cv_results(est.cv_results_, filepath = self.result_dir + "/" + self.model + "_cv_results.csv")
        ...
        if hasattr(est_pipe.named_steps['predictor'], 'best_params_'):
            ...
            self.save_scores(est_pipe.named_steps['predictor'].best_params_, filepath = self.result_dir + "/" + self.model + "_best_params.json")
...
#                                   if best_model.feature_importances_[i] != 0}
            ...
            feat_import_df.to_csv(self.result_dir + "/" + self.model + "_feature_importances.csv", index = False)
            print("Feature Importances:\n{}".format(feat_import_df))
        print("Best Model:\n{}".format(best_model))
        
        est_pipe.steps[-1] = ('predictor', best_model)      
        
        dump(est_pipe, os.path.join(self.result_dir, self.model + '_best_model.joblib'))
        # impute missing values in X for ROC plot
#        self.imp = est_pipe.named_steps['imputer']
#        X = self.imp.transform(X)

        self.plot_roc(est_pipe, X, y, cv, filepath = self.result_dir +'/auc_plot_' + self.model + '.png')
...
#        return scores, best_model
    ...
    def predict_models_from_groups(self, groups, models, cv=5, mode="classification", score_name=None, 
...
#            }
    ...
    def predict_on_test(self, models, test_file = None, out_dir = None):
        ...
        for model_name in models:
            ...
            if model_name == 'lasso':
                ...
                probas_ = model.predict_proba(test_X.as_matrix())
                probas_ = probas_[:, 1]
    ...
    def plot_roc(self, best_model, X, y, cv, filepath):
        ...
        if self.model == 'lasso':
            ...
            coef_dataframe.to_csv('{}/{}_coefficients.csv'.format(self.result_dir, self.model), index = False)
        ...
        for train, test in cv.split(X, y):
            ...
            tr_probas_ = 0
            probas_ = 0
            fold_fit = best_model.fit(X.iloc[train].as_matrix(), y.iloc[train])
            if self.model == 'lasso':
                tr_probas_ = fold_fit.predict(X.iloc[train])
                probas_ = fold_fit.predict(X.iloc[test])
                
            else:
                tr_probas_ = fold_fit.predict_proba(X.iloc[train].as_matrix())
                tr_probas_ = tr_probas_[:, 1]
                probas_ = fold_fit.predict_proba(X.iloc[test].as_matrix())
                probas_ = probas_[:, 1]
            
            tr_result = pd.DataFrame({'index': train, 
                                        'y': y.iloc[train], 
                                        'est_prob': tr_probas_})
            tr_result.to_csv(self.result_dir + '/train_predicted_probs_' + self.model + '_fold_' + str(i) + '.csv', index = False)
            cv_result = pd.DataFrame({'index': test, 
                                        'y': y.iloc[test], 
                                        'est_prob': probas_})
            cv_result.to_csv(self.result_dir + '/predicted_probs_' + self.model + '_fold_' + str(i) + '.csv', index = False)
        ...
        plt.title('Receiver operating characteristic for {}'.format(self.model))
        plt.legend(loc="lower right")
        fig = plt.gcf()
        fig.set_size_inches(6, 4.5)
        fig.savefig(self.result_dir +'/auc_plot_' + self.model + '.png') #str(int(round(time.time() * 1000))) +
...
#         plt.savefig(os.path.join(self.result_dir, 'all_results.png'))
        ...
        for model_type in model_types:
            ...
            if model_type == 'PCE':
                ...
                plt.plot(mean_fpr, mean_tpr,
                         label=r'Mean ROC for %s (AUC = %0.3f $\pm$ %0.3f)' % (model_type, mean_auc, std_auc),
                         lw=2, alpha=.5)
    
            rts[model_type] = roc_auc
    ...
    #         mean_fpr = np.linspace(0, 1, 100)
            ...
            if model_type == 'PCE':
                ...
                # tpr[-1] = 1.0
                plt.plot(fpr, tpr,
                         label=r'ROC for %s (AUC = %0.3f)' % (model_type, roc_auc),
                         lw=2, alpha=.5)
